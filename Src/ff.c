...
static
int move_window (        /* TRUE: successful, FALSE: failed */
...
#if !_FS_READONLY
static
int put_cluster (        /* TRUE: successful, FALSE: failed */
    FATFS *fs,            /* File system object */
...
)
{
    ...
    case FS_FAT12 :
        bc = (WORD)clust * 3 / 2;
        if (!move_window(fs, fatsect + (bc / S_SIZ))) return 0;
        p = &fs->win[bc & (S_SIZ - 1)];
...
#if !_FS_READONLY
static
int remove_chain (        /* TRUE: successful, FALSE: failed */
    FATFS *fs,            /* File system object */
...
static
int next_dir_entry (    /* TRUE: successful, FALSE: could not move next */
